      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="grid gap-3">
          ...
          <ul className="max-h-32 space-y-1 overflow-y-auto text-sm">
            ...
            {lines.length === 0 && <li className="text-sm text-muted-foreground">No line items yet.</li>}
          </ul>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Subtotal + 5% tax (server computes final)</span>
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={!patientId || lines.length === 0 || createInvoice.isPending}>
            {createInvoice.isPending ? "Creating…" : `Create ${formatINR(total)}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
